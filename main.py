...
def scan_for_secrets(file_path, rel=None):
    """Regex the file for hardcoded secrets / exposed API tokens."""
    rel = rel if rel is not None else file_path
            ...
            for line_number, line in enumerate(file, 1):
                if context_regex.search(line):
                    findings.append(make_finding(
                        ...
                        line.strip()[:100], file=rel, line=line_number,
                    ))
...
def scan_jwt_config(file_path, rel=None):
...
def scan_route_logic(file_path, rel=None):
    ...
    if not any(word in rel_lower for word in sensitive_keywords):
        return []

    findings = []
    has_auth = False
    has_owner_check = False
    is_parameterized = "[id]" in rel_lower
...
def scan_plaintext_passwords(file_path, rel=None):
...
def scan_insecure_storage(file_path, rel=None):
...
def scan_sql_injection(file_path, rel=None):
...
def scan_stub_code(file_path, rel=None):
    """Flag placeholder / stub code (TODO/FIXME, fake auth, not-implemented) left in source."""
    rel = rel if rel is not None else file_path
    if not file_path.endswith((".js", ".jsx", ".ts", ".tsx")):
        return []
        ...
        with open(file_path, "r", encoding="utf-8") as file:
            for line_number, line in enumerate(file, 1):
                if stub_regex.search(line):
                    findings.append(make_finding(
                        "static", "Stub / Placeholder Code",
                        "Placeholder / stub code left in source",
                        line.strip()[:100], file=rel, line=line_number,
                    ))
    except Exception:
        pass
...
def run_static_scan(target_dir, config, log=print):
    ...
    for file_path in files:
        ...
        for detector in STATIC_DETECTORS:
            findings.extend(detector(file_path, rel))
...
class DynamicFuzzer:
    ...
    def check_security_headers(self, path="/"):
...
def severity_bar(summary):
    """Build a proportional severity bar for the report banner."""
    total = summary["total_findings"]
    if not total:
        return f'<i style="width:100%;background:{SEVERITY_HEX["MINIMAL"]}"></i>'
    parts = []
    for sev in SEVERITY_ORDER:
        n = summary["by_severity"][sev]
        if n:
            pct = 100 * n / total
            parts.append(f'<i style="width:{pct:.1f}%;background:{SEVERITY_HEX[sev]}"></i>')
    return "".join(parts)
